...
export default function JobCardsPage() {
  ...
  const cardsList = jobCardsQuery.data?.data ?? [];
  ...
  const columns = useMemo<ColumnDef<JobCard>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => `${row.original.completedWeight} kg`,
      },
      {
        id: 'actions',
        ...
        <Card className="glass-card border-slate-800 bg-slate-900/40">
          ...
          <CardContent>
            <QueryStateWrapper
              ...
              error={jobCardsQuery.error}
              retry={jobCardsQuery.refetch}
              isEmpty={cardsList.length === 0}
              emptyTitle="No Job Cards Found"
              emptyDescription="No job cards match the filtered criteria."
            >
              <div className="space-y-4">
                <DataTable columns={columns} data={cardsList} />

                {jobCardsQuery.data && jobCardsQuery.data.totalPages > 1 && (
                  <PaginationControls
                    page={page}
                    totalPages={jobCardsQuery.data.totalPages}
                    onPrevious={() => setPage((current) => Math.max(current - 1, 1))}
                    onNext={() =>
                      setPage((current) =>
                        Math.min(current + 1, jobCardsQuery.data?.totalPages ?? current),
                      )
                    }
                  />
                )}
