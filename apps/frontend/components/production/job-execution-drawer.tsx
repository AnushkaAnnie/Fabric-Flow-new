...
export function JobExecutionDrawer({
  ...
  const completedWeightRef = useRef<HTMLInputElement>(null);
  const latestJob = jobs.find((item) => item.id === job?.id) ?? job;

  const startMutation = useStartJob(() => onOpenChange(false));
  const completeMutation = useCompleteJob(() => onOpenChange(false));
  ...
  const handleCompleteClick = () => {
    const weight = Number(completedWeightRef.current?.value);
            ...
            <div className="flex flex-col gap-4">
              {latestJob.status === 'ISSUED' && (
                <Button
                  ...
                  onClick={() => startMutation.mutate(latestJob.id)}
              ...
              {latestJob.status === 'IN_PROGRESS' && (
                <div className="space-y-4">
                  <div>
                    ...
                    <Input
                      key={`${latestJob.id}-${latestJob.targetWeight}`}
                      ref={completedWeightRef}
                      type="number"
                      step="0.01"
                      required
                      defaultValue={latestJob.targetWeight}
                      disabled={isProcessing}
