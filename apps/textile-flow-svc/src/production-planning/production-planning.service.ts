...
export class ProductionPlanningService {
  ...
  async generateJobCardNo() {
    ...
    return `JOB-${new Date().getFullYear()}-${String(count + 1).padStart(
      4,
      '0',
    )}`;
  }
  ...
  async validateMachineCapacity({
    ...
    const existing = await this.prisma.jobCard.count({
      where: {
        machineNo,
        ...
        status: {
          not: JobCardStatus.COMPLETED,
        },
      },
    });

    if (existing >= 5) {
      throw new BadRequestException('Machine capacity exceeded');
    }
  }
  ...
  async validateShiftCollision({
    ...
    const existing = await this.prisma.jobCard.findFirst({
      where: {
        ...
        status: {
          in: [JobCardStatus.ISSUED, JobCardStatus.IN_PROGRESS],
        },
      },
    });
  ...
  async createJobCard(dto: CreateJobCardDto) {
    const plan = await this.prisma.productionPlan.findUnique({
      where: {
        id: dto.productionPlanId,
      },
    });
    ...
    const created = await this.prisma.jobCard.create({
      data: {
        jobCardNo: await this.generateJobCardNo(),
        ...
        status: JobCardStatus.ISSUED,
      },
    });

    await this.logEvent({
      productionPlanId: created.productionPlanId,

      jobCardId: created.id,

      eventType: 'JOB_CARD_CREATED',

      message: `Job card ${created.jobCardNo} created`,
    });

    return created;
  }

  async startJobCard(id: number) {
    const existing = await this.prisma.jobCard.findUnique({
      where: { id },
    });
    ...
    if (existing.status === JobCardStatus.COMPLETED) {
      throw new BadRequestException('Completed job card cannot be started');
    }
    ...
    await this.logEvent({
      productionPlanId: updated.productionPlanId,

      jobCardId: updated.id,

      eventType: 'JOB_CARD_STARTED',

      message: `Job card ${updated.jobCardNo} started`,
    });

    return updated;
  }

  async completeJobCard(id: number, completedWeight: number) {
    ...
    if (job.status === JobCardStatus.COMPLETED) {
      throw new BadRequestException('Job card already completed');
    }

    if (completedWeight > job.targetWeight) {
      throw new BadRequestException('Completed weight exceeds target weight');
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const totalCompleted = await tx.jobCard.aggregate({
        where: {
      ...
      const completed = Number(totalCompleted._sum.completedWeight ?? 0);

      const status =
        completed >= job.productionPlan.plannedWeight
          ? ProductionPlanStatus.COMPLETED
          : ProductionPlanStatus.IN_PROGRESS;

      return tx.productionPlan.update({
        where: {
          id: job.productionPlanId,
        },
        ...
        data: {
          ...
          actualEndDate:
            status === ProductionPlanStatus.COMPLETED ? new Date() : null,
        },
      });
    });
    ...
    await this.logEvent({
      ...
      eventType: 'JOB_CARD_COMPLETED',

      message: `Job card ${job.jobCardNo} completed`,
    });

    if (result.status === ProductionPlanStatus.COMPLETED) {
      await this.completeProductionInventory({
        lotNo: job.productionPlan.lotNo,

        quantity: completedWeight,
    ...
    await this.lotTrackerService
      .evaluateLot(job.productionPlan.lotNo)
      .catch(() => {});
  ...
  async getJobCards({
    ...
    if (status) {
      where.status = status as JobCardStatus;
    }
